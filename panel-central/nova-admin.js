// SwitchVP admin dashboard, bilingual (EN/FA) + light/dark, talks to the worker's /admin APIs.
// PANEL_VERSION = the version of these dashboard files (public/). Bump it whenever you change
// the panel, so after uploading you can confirm in the sidebar (under "SwitchVP") that the
// new files are live. The Worker has its own version (shown next to it as "w…").
const PANEL_VERSION = '1.0.0';
const $ = (s) => document.getElementById(s);
let cfg = null;
try { const _t = document.querySelector('.brand .tag'); if (_t) { _t.textContent = 'Panel v' + PANEL_VERSION; _t.title = 'SwitchVP dashboard v' + PANEL_VERSION; } console.log('%cNova panel v' + PANEL_VERSION, 'color:#22d3ee;font-weight:700'); } catch (e) {}

const T = {
  en: { dir:'ltr',
    nav_section:'Management', nav_home:'Dashboard', nav_sub:'Subscriptions', nav_ip:'Clean IPs', nav_isp:'Per-ISP pools', nav_net:'Network & DNS', nav_conn:'Network & IPs', tour_next:'Next', tour_prev:'Back', tour_done:'Done', nav_set:'Settings', nav_guide:'Guide', guide_s:'How to use SwitchVP, step by step.', logout:'Logout', close:'Close',
    ...
    manage_noapi:'Set the Central API (Settings → Advanced) and run your SwitchVP control server to see the fleet here.', manage_err:'Could not reach the central server.',
    ...
    help_isp:'A live count of clean Cloudflare IPs known to work on each Iranian operator (MCI, Irancell, Shatel...), gathered by the SwitchVP Radar scanner. The worker automatically serves each user the best IPs for their own network. Turn on Smart (per-ISP) on the Clean IPs page to populate this.',
    ...
    isp_s:'Live clean-IP counts from the crowd database, by Iranian ISP.', isp_live:'Live pools', isp_hint:'Counts grow as SwitchVP Radar scans on each network.', refresh:'Refresh', isp_noapi:'Set the Smart IP API on the Clean IPs page first.',
    ...
    adv_h:'Advanced (Fragment / ECH)', adv_s:'Optional tuning. Leave blank for defaults. Press Save, then re-import the link.', adv_hint:'ECH needs the ECH toggle on (Security above). Central API enables heartbeat + broadcast announcements when you run a SwitchVP control server.',
    ...
    help_usage:'How much traffic SwitchVP has carried (today / month / year / all-time), plus a daily graph. Numbers are approximate and never cost you anything — just for your own tracking.',
    ...
    help_mode:'How SwitchVP picks the Cloudflare IPs your config uses: Smart (best per-ISP, recommended), Custom (your own list), or Random.',
  ...
  fa: { dir:'rtl',
    nav_section:'مدیریت', nav_home:'داشبورد', nav_sub:'اشتراک‌ها', nav_ip:'آی‌پی‌های تمیز', nav_isp:'مخزن هر اپراتور', nav_net:'شبکه و DNS', nav_conn:'شبکه و آی‌پی‌ها', tour_next:'بعدی', tour_prev:'قبلی', tour_done:'پایان', nav_set:'تنظیمات', nav_guide:'راهنما', guide_s:'راهنمای گام‌به‌گام استفاده از SwitchVP.', logout:'خروج', close:'بستن',
    ...
    help_isp:'تعداد زنده‌ی آی‌پی‌های تمیز کلودفلر که روی هر اپراتور ایرانی (همراه اول، ایرانسل، شاتل...) کار می‌کنند و توسط اسکنر SwitchVP Radar جمع‌آوری شده‌اند. ورکر به‌طور خودکار به هر کاربر بهترین آی‌پی‌ها را برای شبکه‌ی خودش می‌دهد. برای پر شدن این صفحه، در صفحه‌ی آی‌پی‌های تمیز حالت هوشمند را روشن کنید.',
    ...
    isp_s:'تعداد زنده‌ی آی‌پی‌های تمیز از پایگاه‌داده، به تفکیک اپراتور ایرانی.', isp_live:'مخزن‌های زنده', isp_hint:'تعدادها با اسکن SwitchVP Radar روی هر شبکه بیشتر می‌شود.', refresh:'به‌روزرسانی', isp_noapi:'ابتدا API آی‌پی هوشمند را در صفحه‌ی آی‌پی‌های تمیز تنظیم کنید.',
...
<p style="font-size:13px">This is the control panel for your own SwitchVP. From here you hand out a <b>subscription link</b> that any VPN app can use, and you tune how it works. You don't need to understand the technical parts. The steps below are all most people ever touch. Use the menu on the left to move between pages.</p>
</div>
...
<p style="font-size:13px">If your ISP blocks by reading the <b>SNI</b> (the server name in the TLS hello), there are three layers — the first two are built into SwitchVP, the third is a device-side app:</p>
...
<div class="gstep"><span class="n">•</span><span><b>Auto-provision</b> (recommended): at <b>switchvp.example &rarr; Deploy</b>, paste a Cloudflare API token. The system builds the worker, database, domain and clean IPs and hands you a subscription link + QR.</span></div>
...
<div class="card"><h4>📡 Per-ISP pools &amp; SwitchVP Radar</h4>
<p style="font-size:13px">This page shows how many clean Cloudflare IPs are known to work on each Iranian operator (MCI, Irancell, Shatel...). The data comes from the <b>SwitchVP Radar</b> scanner. To fill it:</p>
<div class="gstep"><span class="n">1</span><span>On the <b>Clean IPs</b> page choose <b>Smart (per-ISP)</b> and set the API to <code>switchvp-deploy.pages.dev/api/pool</code>, then Save.</span></div>
<div class="gstep"><span class="n">2</span><span>Run <b>SwitchVP Radar</b> (desktop or Android) <b>on an Iranian network</b>. It scans Cloudflare IPs and uploads the fastest ones for that operator.</span></div>
...
<p style="font-size:13px">On <b>Subscriptions</b>, the <b>Free-service notice</b> is <b>always on</b> and its <b>message is fixed</b> — a labelled node is baked into every config so people always see it's the free SwitchVP service. It can't be turned off or changed by anyone (so resellers can't strip or rebrand it).</p>
</div>
...
<p style="font-size:13px">این پنل کنترل پروکسی SwitchVP شماست. از اینجا یک <b>لینک اشتراک</b> می‌دهید که هر برنامه‌ی VPN می‌تواند از آن استفاده کند، و نحوه‌ی کارکرد آن را تنظیم می‌کنید. لازم نیست بخش‌های فنی را بفهمید؛ مراحل زیر همه‌ی چیزی است که بیشتر افراد به آن نیاز دارند. برای جابه‌جایی بین صفحه‌ها از منوی سمت راست استفاده کنید.</p>
</div>
...
<div class="gstep"><span class="n">•</span><span><b>راه‌اندازی خودکار</b> (پیشنهادی): در <b>switchvp.example &larr; استقرار</b> یک توکن Cloudflare بچسبانید. سیستم ورکر، دیتابیس، دامنه و آی‌پی تمیز را می‌سازد و لینک اشتراک + QR به شما می‌دهد.</span></div>
...
<div class="card"><h4>📡 مخزن هر اپراتور و SwitchVP Radar</h4>
<p style="font-size:13px">این صفحه نشان می‌دهد برای هر اپراتور ایرانی (همراه اول، ایرانسل، شاتل...) چند آی‌پی تمیز کلودفلر شناخته‌شده است. داده‌ها از اسکنر <b>SwitchVP Radar</b> می‌آید. برای پر کردن آن:</p>
<div class="gstep"><span class="n">۱</span><span>در صفحه‌ی <b>آی‌پی‌های تمیز</b> حالت <b>هوشمند (هر اپراتور)</b> را انتخاب و API را روی <code>switchvp-deploy.pages.dev/api/pool</code> بگذارید و ذخیره کنید.</span></div>
<div class="gstep"><span class="n">۲</span><span><b>SwitchVP Radar</b> (دسکتاپ یا اندروید) را <b>روی یک شبکه‌ی ایرانی</b> اجرا کنید. آی‌پی‌های کلودفلر را اسکن و سریع‌ترین‌ها را برای آن اپراتور آپلود می‌کند.</span></div>
...
function loadBanner(){ const el=$('switchvp-banner'); if(el) el.style.display='none';
...
async function loadAll(){
  ...
  [['a-h','hiddify://import/'+slB64,slB64],['a-v','v2rayng://install-config?url='+encodeURIComponent(slB64),slB64],['a-c','clash://install-config?url='+encodeURIComponent(slClash),slClash],['a-k','karing://install-config?url='+encodeURIComponent(slClash)+'&name=SwitchVP',slClash],['a-s','sing-box://import-remote-profile?url='+encodeURIComponent(slSb),slSb]]
  ...
  if($('f-noticetext')) $('f-noticetext').value='🎁 Free service — switchvp.example | سرویس رایگان نوا'; // fixed
  // Default the Smart IP API to SwitchVP's shared per-ISP database so users get it out of the box.
  $('f-poolapi').value=cfg.POOL_API||'https://switchvp-deploy.pages.dev/api/pool';
...
async function loadStatus(){
  ...
  try{ const r=await fetch('/admin/system.json?_t='+Date.now()); const j=await r.json();
    ...
    const tag=document.querySelector('.brand .tag'); if(tag){ tag.textContent='Panel v'+PANEL_VERSION+(j.version?' · w'+j.version:''); tag.title='SwitchVP dashboard v'+PANEL_VERSION+(j.version?' · worker '+j.version:'')+(j.instanceId?' · instance '+j.instanceId:''); }
...
async function startScan(onProg){
  ...
  if(src==='random'){ while(ips.length<total){ const ip=randCfIp(); if(!seen.has(ip)){ seen.add(ip); ips.push(ip); } } }
    ...
    // Pull candidate IPs from a SwitchVP radar source, then latency-test them in the browser.
...
if($('sc-use')) $('sc-use').onclick=async()=>{
  ...
  const lines=scanResults.map(r=>r.ip+':'+r.port+'#SwitchVP-'+nova5());
...
(function(){
  try{
    ...
    function show(){
      if (document.getElementById('switchvp-pwa')) return;
      const b=document.createElement('div'); b.id='switchvp-pwa';
      ...
      const f=fa();
      const txt = isIOS
        ? (f?'نصب نوا روی صفحه اصلی: دکمهٔ اشتراک‌گذاری سافاری (⬆️) ← «Add to Home Screen»':'Install SwitchVP: Safari Share (⬆️) → “Add to Home Screen”')
        : (f?'نصب «نوا پروکسی» روی گوشی':'Install SwitchVP on your phone');
...
if($('backup-export')) $('backup-export').onclick=async()=>{
  ...
  try{
    ...
    const blob=new Blob([JSON.stringify({switchvp:1,exportedAt:new Date().toISOString(),config:c,network:n},null,2)],{type:'application/json'});
    const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='switchvp-backup-'+new Date().toISOString().slice(0,10)+'.json'; a.click(); URL.revokeObjectURL(a.href);
